...
2. The foreign keys (**FK**) are noted in the tables, and the associated column
   ...
   the form `*_id`. The main exceptions are one-to-one tables where `id` is
   both the primary key and the foreign key, e.g. [`kernel`](#kernel) and
   [`scratch`](#scratch), which each reuse [`variable_space`](#variable_space)
   ids.
...
the YAML/TOML/JSON file), optional `tags` for organization, and the version of
...
It does not carry an `id`: the identifier belongs to this table's `id` column,
assigned by the sequence when the row is inserted, and an order file may not
declare one.
...
in the `variable_space` table as well. If the kernel fails (non-zero `exit_code`),
the `error` column stores the associated error message.
...
out-of-the-box, future versions may include other kernel implementations or
...
Each exception is stored as a row
...
- INDEX `equilibrium_space_log_fo2_idx` on (`log_fO2`)
...
the molecular composition of the fluid
...
1. Since mass is not transferred in the `"eq3"` stage, columns such as
   `reactant_mass_reacted` and `reactant_mass_remaining` will be `NULL`.

Less commonly queried solution properties (such as `mole_fraction_water`,
`log_gamma_water`, `pe`, `Ah`, charge-balance quantities, solid mass and volume
changes, etc.) are stored in the `custom_properties` column. Each kernel may
also compute and store additional non-standard properties there.
...
be associated with some number of rows in the `equilibrium_end_members` table,
one for each end member. These rows record the `name`, saturation index
...
- INDEX `equilibrium_redox_reactions_log_fo2_idx` on (`log_fO2`)

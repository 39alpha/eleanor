...
The tables in the ``campaign.sql`` database are ``vs``, ``es``, and ``orders``. They store different information about 
...
The ``es`` table records the result of the equilibrium calculations, it contains the activities of all the components of 

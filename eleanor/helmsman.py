...
def main():
	...
	try:
		### was an order number given as an argument?
		###	order id to be retrieved in vs
		ord_id = sys.argv[1] 								
	except:
		sys.exit('give order number')
		### no order number explicitly called for
		### search for the last order listed in vs that has not been 
		### run
	...
	### retrieve column names for this praticualr table. This MAY be
	### differnet from project to project if i deicde on different 
	### basis for different campaigns.
	try:
	...
	with multiprocessing.Pool(processes=cores) as pool:
		results = pool.starmap(sailor, zip([order_path]*len(rec), 
			[date]*len(rec), rec, [elements]*len(rec), [col_names]*len(rec)))
...
def sailor(order_path, date, dat, elements, col_names):
	...
	state_dict = {}
	for i, j in zip(camp.vs_state.keys(), 
		dat[6 + 2*rnt_n : 6 + 2*rnt_n + state_n]):
		state_dict[i] = j

	basis_dict = {}
	for i, j in zip(camp.vs_basis.keys(), 
		dat[6 + 2*rnt_n + state_n : 6 + 2*rnt_n + state_n + basis_n]):
		basis_dict[i] = j
	...
	if not os.path.isfile(file[:-1] + 'p'):
		...
		reset_sailor(order_path, start, conn, camp.name, file, dat[0], 30, delete_local = delete_after_running)
	...
	try:
		...
		print('{}\n  {}\n'.format(file ,e))
		reset_sailor(order_path, start, conn, camp.name, file, dat[0], 31, delete_local = delete_after_running)
		return
	...
	if not os.path.isfile(file[:-2] + '6o'):
		reset_sailor(order_path, start, conn, camp.name, file, dat[0], 60, delete_local = delete_after_running)
		return

...
class TestUtils(TestCase):
    ...
    def test_progress_init_and_join(self):
        """
        Ensure that :class:`Progress` initializes and joins process/queue resources correctly.
        """
        class FakeQueue:
            def __init__(self):
                self.items = []
        ...
        manager = FakeManager()
        with mock.patch.object(util, "Process", FakeProcess):
            progress = util.Progress(manager)
            self.assertTrue(progress.process.started)
            self.assertEqual(progress.samples, 0)

            progress.join()
            self.assertEqual(progress.queue.items, [False])
            self.assertTrue(progress.queue.join_called)
            self.assertTrue(progress.process.joined)

    def test_progress_listen_total_update_and_close(self):
        """
        Ensure that :meth:`Progress.listen` handles total updates and closes progress cleanly.
        """
        events = []
        ...
        class FakeTqdm:
            def __init__(self, total, unit, colour):
                self.total = total
                events.append(("init", total, unit, colour))

            def update(self):
                events.append(("update",))

            def refresh(self):
                events.append(("refresh", self.total))

            def close(self):
                events.append(("close",))

        progress = object.__new__(util.Progress)
        progress.samples = 0
        progress.no_total_update = False
        progress.queue = FakeQueue([5, 2, False])

        with mock.patch.object(util, "tqdm", FakeTqdm):
            progress.listen()

        self.assertEqual(progress.samples, 7)
        self.assertEqual(progress.queue.task_done_count, 3)
        self.assertEqual(events[0], ("init", 5, " systems", "#ec5c29"))
        self.assertIn(("refresh", 7), events)
        self.assertEqual(events[-1], ("close",))

    def test_progress_listen_boolean_update_and_no_total_update(self):
        """
        Ensure that :meth:`Progress.listen` handles boolean updates and no-total-update mode.
        """
        class FakeQueue:
            def __init__(self, messages):
                self.messages = list(messages)
        ...
        # Covers bool-True path and progress initialization inside that branch.
        p0 = object.__new__(util.Progress)
        p0.samples = 0
        p0.no_total_update = False
        p0.queue = FakeQueue([True, False])

        with mock.patch.object(util, "tqdm", FakeTqdm):
            p0.listen()

        self.assertEqual(p0.queue.task_done_count, 2)
        self.assertEqual(FakeTqdm.instances[-1].update_calls, 1)
        self.assertEqual(FakeTqdm.instances[-1].close_calls, 1)

        # Covers no_total_update=True path for numeric message after first total.
        p1 = object.__new__(util.Progress)
        p1.samples = 0
        p1.no_total_update = True
        p1.queue = FakeQueue([4, 3, False])

        with mock.patch.object(util, "tqdm", FakeTqdm):
            p1.listen()

        self.assertEqual(p1.samples, 4)
        self.assertEqual(p1.queue.task_done_count, 3)
        self.assertEqual(FakeTqdm.instances[-1].refresh_calls, 0)

    def test_progress_listen_additional_total_when_progress_none(self):
        """
        Ensure that :meth:`Progress.listen` handles additional total updates in current flow.
        """
        events = []
        ...
        p = object.__new__(util.Progress)
        p.samples = 2
        p.no_total_update = False
        p.queue = FakeQueue([3, False])

        with mock.patch.object(util, "tqdm", FakeTqdm):
            p.listen()

        self.assertEqual(p.samples, 3)
        self.assertEqual(p.queue.task_done_count, 2)
        self.assertEqual(events[0], ("init", 3, " systems", "#ec5c29"))
        self.assertEqual(events[-1], ("close",))

    def test_progress_listen_total_update_branch_when_tqdm_returns_none(self):
        """
        Ensure that :meth:`Progress.listen` handles total updates when tqdm construction yields None.
        """
        class FakeQueue:
            def __init__(self, messages):
                self.messages = list(messages)
        ...
        p.samples = 0
        p.no_total_update = False
        p.queue = FakeQueue([5, 2, False])

        with mock.patch.object(util, "tqdm", return_value=None):
            p.listen()

        self.assertEqual(p.samples, 7)
        self.assertEqual(p.queue.task_done_count, 3)

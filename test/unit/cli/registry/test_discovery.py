...
def test_discovery_registers_entry_points(mocker: MockerFixture, makeEntryPoint: Ctor, spec: SimplePluginSpec):
...
@pytest.mark.usefixtures("clean_registry")
def test_discovery_raises_on_load_failure(mocker: MockerFixture, makeEntryPoint: Ctor):
    def fail():
    ...
    with pytest.raises(EleanorException, match="failed to load cli entry point 'broken'"):
        _ = available_cli_commands()


@pytest.mark.usefixtures("clean_registry")
def test_discovery_raises_on_non_spec_entry_point(mocker: MockerFixture, makeEntryPoint: Ctor):

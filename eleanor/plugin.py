...
Each extension point (executor, kernel, navigator, transformer) instantiates a
:class:`PluginRegistry` seeded with the built-in factories it ships with, and
consumers go through the registry instead of importing implementations
directly. Third-party plugins advertise themselves through a per-extension
``eleanor.<kinds>`` entry-point group; see :mod:`eleanor.executor.registry`,
:mod:`eleanor.kernel.registry`, :mod:`eleanor.navigator.registry`, and
:mod:`eleanor.transformers.registry` for the concrete wiring.

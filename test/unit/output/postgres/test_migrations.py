...
def test_discover_parses_notxn_file(fake_migrations_pkg: Path) -> None:
    ...
    (fake_migrations_pkg / "0001_add_foo_idx.notxn.sql").write_text(sql, encoding="utf-8")
    result = migrations.discover()
    assert len(result) == 1
    m = result[0]
    assert m.version == 1
...
def test_discover_ignores_non_sql_files(fake_migrations_pkg: Path) -> None:
    ...
    (fake_migrations_pkg / "0001_initial_schema.sql").write_text("SELECT 1;", encoding="utf-8")
    result = migrations.discover()
    assert len(result) == 1
...
def test_filename_regex_accepts_long_underscore_slug(fake_migrations_pkg: Path) -> None:
    ...
    for i in range(1, 42):
        (fake_migrations_pkg / f"{i:04d}_placeholder.sql").write_text("SELECT 1;", encoding="utf-8")
    result = migrations.discover()

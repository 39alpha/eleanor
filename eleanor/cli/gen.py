...
Templates live as plain files under `eleanor/cli/templates/` and are loaded via
`importlib.resources` so they work both from source checkouts and installed
wheels.
"""
...
def validate_template(target: str, fmt: str) -> Config | Order:
    """Load a template and parse it through `load_config` or `load_order`.
...
@gen.command("config")
@click.option("-f", "--format", "fmt", type=click.Choice(FORMATS), default="yaml", help="Output format.")

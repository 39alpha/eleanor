...
class TestSchemaDdlEmission(TestCase):
    ...
    def test_to_drop_constraint_sql_emits_quoted_alter_if_exists(self):
        ...
        self.assertEqual(
            ...
            'ALTER TABLE "variable_space" '
            'DROP CONSTRAINT IF EXISTS "variable_space_order_id_fkey"',
        )
...
class TestConverterShapes(TestCase):
    ...
    def test_es_pure_solid_to_row_replaces_none_with_neg_inf(self):
        ...
        solid = core_es.PureSolid(
            name='Calcite', log_qk=0.2, affinity=1.0,
    ...
    def test_bulk_insert_uses_copy_above_threshold(self):
        ...
        with (
            ...
            repositories._bulk_insert(  # pyright: ignore[reportPrivateUsage]
                cursor, 'equilibrium_elements', rows,
            )
...
class TestRepositoryErrorPaths(TestCase):
    ...
    def test_setup_schema_wires_connect_to_ensure_schema(self):
        ...
        with (
            mock.patch.object(
                repositories.connection, 'connect', return_value=fake_conn,
            ) as connect,
    ...
    def test_insert_variable_space_raises_when_returning_yields_nothing(self):
        ...
        with self.assertRaisesRegex(
            EleanorException, 'variable_space INSERT did not return an id',
        ):
            _ = repositories._insert_variable_space_and_pair(  # pyright: ignore[reportPrivateUsage]
                cursor, point, order_id=1,
            )
...
class TestConverterErrorAndReactantPaths(TestCase):
            ...
            (
                converters.special_reactant_to_row(
                    core_vs.SpecialReactant(
                        name='s', log_moles=0.0, titration_rate=1.0, composition=[],
                    ),
                    variable_space_id=7,
                ),
            ...
            (
                converters.solid_solution_reactant_to_row(
                    core_vs.SolidSolutionReactant(
                        name='ss', log_moles=0.0, titration_rate=1.0, end_members=[],
                    ),
                    variable_space_id=7,
                ),
            ...
            (
                converters.glass_reactant_to_row(
                    core_vs.GlassReactant(
                        name='glass', log_moles=0.0, titration_rate=1.0, oxides=[],
                    ),
                    variable_space_id=7,
                ),
    ...
    def test_glass_reactant_oxide_and_composition_rows(self):
        ...
        comp_expected = {
            c.name for c in schema.GLASS_REACTANT_OXIDE_COMPOSITIONS.columns if not c.identity
        }
...
class TestConnectionCacheBehaviour(TestCase):
    ...
    def test_close_all_connections_drains_cache_even_when_close_raises(self):
        ...
        self.assertEqual(
            len(set(connection._connections.keys()) & {  # pyright: ignore[reportPrivateUsage]
                (cfg_a, os.getpid()),
                (cfg_b, os.getpid()),
            }),
            0,
...
class TestBulkLoadLifecycle(TestCase):
    ...
    def test_drop_indexes_introspects_fks_and_drops_named_checks_and_indexes(self):
        ...
        self.assertEqual(
            fk_calls_seen, [t.name for t in schema.TABLES],
        )
        ...
        for table in schema.TABLES:
            ...
            for idx in table.indexes:
                self.assertTrue(
                    any(
                        f'DROP INDEX IF EXISTS "{idx.name}"' in s
                        for s in statements
                    ),

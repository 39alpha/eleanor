...
def Radar(camp,
    ...
    with camp.working_directory():
        ...
        for order in ord_id:
            ...
            df = retrieve_combined_records(conn,
                                           vs_sp,
                                           es_sp,
                                           limit=None,
                                           ord_id=order,
        ...
        add_text = '\n'.join([
            f"campaign: {camp.name}", f"order/s: {ord_id}", f"data: {date}",
            f"n = {len(df)}", f"x = {x_sp}", f"y = {y_sp}", f"z = {z_sp}",
            f"sql 'where' claus: {where}", f"notes: {description}"
        ])
...
def group_by_solids(conn, camp_name, ord_id):
    ...
    """
    lines = grab_lines(
        os.path.join(PWD, '{}_huffer'.format(camp_name), 'test.3o'))
    solids = []
    solid_solutions = []
    ...
    for _ in range(len(lines)):
        if re.findall('^\n', lines[_]):
            ...
            while not re.findall(
                    '^\n',
                    lines[_ +
                          x]):  # signals the end of the solid solutions block
        ...
        elif '           --- Saturation States of Solid Solutions ---' in lines[
                _]:
            x = 4
            while not re.findall(
                    '^\n',
    ...
    all_rec = retrieve_postgres_record(
        conn,
        'select {} from {}_es where ord = {}'.format(solids_sql, camp_name,
                                                     ord_id))
    ...
    unique_combinations = [
        list(x)
        for x in set(tuple(x) for x in solid_combinations) if list(x) != []
    ] + ['']
...
def solid_groups(conn, pwd, camp_name, ord_id, out='assemblages'):
    ...
    if out == 'assemblages':
        ...
        solids_sql = ",".join([f'"{_}"' for _ in all_precip])
        all_rec = retrieve_postgres_record(
            conn, 'select {} from {}_es where ord = {}'.format(
                solids_sql, camp_name, ord_id))
        ...
        # unique mineral co-precipiation occrrances in  order # ord_id
        unique_combinations = [
            list(x) for x in set(tuple(x)
                                 for x in solid_combinations) if list(x) != []
        ] + ['']
...
def get_continuous_cmap(hex_list, float_list=None):
    ...
    for num, col in enumerate(['red', 'green', 'blue']):
        ...
        cdict[col] = col_list
    cmp = matplotlib.colors.LinearSegmentedColormap('my_cmp',
                                                    segmentdata=cdict,
                                                    N=256)
    return cmp
...
color_dict = {
    '1': [
        '#E76F51', '#264653', '#2A9D8F', '#F4A261', '#E9C46A', '#100B00',
        '#A5CBC3', '#3B341f', '#2F004F'
    ],
    '2': [
    ...
    '3': [
        '#7CEA9C', '#F433AB', '#2E5EAA', '#593959', '#F0C808', '#DD1C1A',
        '#F05365', '#FF9B42', '#B2945B', '#000000', '#FF0000', '#FFA500',
...
RAINBOW_BLK = LinearSegmentedColormap.from_list("mycmap", [
    "#020004", "#75228f", "#3e53d2", "#4eb01f", "#ffd805", "#fd9108", "#dd2823"
])
...
blu_to_orng = [
    "#47eaff", "#2bbae0", "#2f8fd1", "#3363c2", "#3a0ca3", "#9d2a52",
    "#ff4800", "#ff7900", "#ffa224", "#ffcb47"
]

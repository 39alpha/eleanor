...
class TestDBComms(TestCase):
    ...
    def test_retrieve_records(self):
        with TemporaryDirectory() as root:
            ...
            conn.executemany('INSERT INTO `vs` VALUES (?)', [(1, ), (2, ), (3, )])
    ...
    def test_retrieve_combined_records(self):
        with TemporaryDirectory() as root:
            ...
            with self.assertRaises(sqlite3.Error):
                dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'])
            ...
            dbc.execute_query(conn, 'CREATE TABLE `es6` (`uuid` VARCHAR(256), `ord` SMALLINT, `Z` REAL, `W` REAL)')

            df = dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'])
            self.assertEqual(sorted(df.columns), ['X_v', 'Y_e3', 'Z_e6'])
            self.assertEqual(len(df), 0)

            conn.executemany('INSERT INTO `vs` VALUES (?, ?, ?, ?)', [('a', 1, 1.1, 2.1), ('b', 1, 1.2, 2.2),
                                                                      ('c', 2, 1.3, 2.3)])
            ...
            conn.executemany('INSERT INTO `es3` VALUES (?, ?, ?, ?)', [('a', 1, 3.1, 4.1), ('b', 1, 3.2, 4.2),
                                                                       ('c', 2, 3.3, 4.3)])
            ...
            conn.executemany('INSERT INTO `es6` VALUES (?, ?, ?, ?)', [('a', 1, 5.1, 6.1), ('b', 1, 5.2, 6.2)])

            df = dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'])
            self.assertEqual(sorted(df.columns), ['X_v', 'Y_e3', 'Z_e6'])
            self.assertEqual(len(df), 2)
            self.assertEqual([tuple(r) for r in df.to_numpy()], [(1.1, 3.1, 5.1), (1.2, 3.2, 5.2)])

            conn.executemany('INSERT INTO `es6` VALUES (?, ?, ?, ?)', [('c', 2, 5.3, 6.3)])

            df = dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'])
            self.assertEqual(sorted(df.columns), ['X_v', 'Y_e3', 'Z_e6'])
            self.assertEqual(len(df), 3)
            self.assertEqual([tuple(r) for r in df.to_numpy()], [(1.1, 3.1, 5.1), (1.2, 3.2, 5.2), (1.3, 3.3, 5.3)])

            df = dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'], ord_id=2)
            self.assertEqual(sorted(df.columns), ['X_v', 'Y_e3', 'Z_e6'])
            self.assertEqual(len(df), 1)
            self.assertEqual([tuple(r) for r in df.to_numpy()], [(1.3, 3.3, 5.3)])

            with self.campaign.working_directory() as campdir:
                expected = dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'], fname='data.json')
                ...
                expected = dbc.retrieve_combined_records(conn, ['X'], ['Y'], ['Z'], fname='data.csv')

        ...
        for s in species.values():
            for substrate in s.dissociation.substrates.keys():
                if substrate not in self.species_names:
                    raise Exception(f'{s} has a substrate ({substrate}) in its dissocation reaction ({s.dissociation}) that does not appear in the elements or any species sections') # noqa E501

            for product in s.dissociation.products.keys():
                if product not in self.species_names:
                    raise Exception(f'{s} has a product ({product}) in its dissocation reaction ({s.dissociation}) that does not appear in the elements or any species sections') # noqa E501

            if len(s.logk) != len(self.params.temperatures):
                raise Exception(f'{s} has a different number of log(K) values than the number of values in the data0 temperature grid') # noqa E501
